...
def shuffled_hdf5_batch_generator(state_dataset, action_dataset,
    ...
    # state_dataset is channels_last: (n, board_size, board_size, n_features)
    ...
    while True:
        for data_idx in indices:
            ...
            # get state from dataset and transform it. Each transform acts on the first two
            # (spatial) dimensions and leaves the trailing feature-channel dimension alone.
            state = transform(state_dataset[data_idx])
...
def run_training(cmd_line_args=None):
    ...
    if 'features' in dataset:
        ...
        if isinstance(dataset_features, bytes):
            dataset_features = dataset_features.decode('ascii')
    ...
    # Keras 3 dropped SGD's `decay` argument; reproduce the old lr/(1+decay*step) schedule
    # via a learning-rate schedule instead.
    lr_schedule = InverseTimeDecay(initial_learning_rate=args.learning_rate, decay_steps=1,
                                   decay_rate=args.decay)
    sgd = SGD(learning_rate=lr_schedule)
    # jit_compile=False: XLA's fused conv+bias+activation autotuner fails to find a
    # supported cuDNN algorithm for this architecture/GPU combination (confirmed via direct
    # testing - "Autotuner could not find any supported configs for HLO: ...
    # convBiasActivationForward"). The plain (non-XLA) conv path works fine.
    model.compile(loss='categorical_crossentropy', optimizer=sgd, metrics=["accuracy"],
                  jit_compile=False)

    samples_per_epoch = args.epoch_length or n_train_data
    steps_per_epoch = samples_per_epoch // args.minibatch
    validation_steps = n_val_data // args.minibatch
